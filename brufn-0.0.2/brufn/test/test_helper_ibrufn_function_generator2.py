...
from brufn.helper_ibrufn_function_generator2 import IBRUFNFunctionGenerator
import simplejson as json
from copy import deepcopy
from brufn.utils import print_str_to_file
import shutil

class TestIBRUFNFuntionGenerator2(unittest.TestCase):
...
CONTACTS = [
    ...
    {'from': 2, 'to': 3, 'ts': 2, 'pf': 0.5}
        ...
        func = IBRUFNFunctionGenerator(net, 3, copies, self.WORKING_DIR, 0.5).generate()
...
CONTACTS = [
    ...
    {'from': 3, 'to': 4, 'ts': 2, 'pf': 0.5},
        ...
        for target in targets:
            func = IBRUFNFunctionGenerator(net, target, copies, self.WORKING_DIR, 0.5).generate()
            for c in range(1, copies+1):

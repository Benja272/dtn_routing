...
class ContactPlan:
    ...
    def _get_net(self, f_gen_net, delta_t = 1, output_path=None) -> Net:
        graph_by_slice = dict((t, GraphTS(t, t + delta_t)) for t in range(self.start_time, self.end_time, delta_t))
        for c in self.contacts:
            for t in range(self.start_time, self.end_time, delta_t):
                if c.start_t < t + delta_t and c.end_t > t:
                    graph_by_slice[t].add_contact(c)

        nets = f_gen_net([graph_by_slice[t] for t in sorted(graph_by_slice.keys())])
        net_contacts = []
        for ts in range(len(nets)):
            for c in nets[ts].contacts:
                net_contacts.append(Contact(c.source - 1, c.target - 1, ts, identifier=c.id - 1, begin_time=c.start_t, end_time=c.end_t))

        net = Net(self.node_number, net_contacts)
        if output_path is not None:
            net.print_to_file(output_path)
            net.to_dot(output_path)
    ...
    def generate_statistics(self):
        ...
        statistics['contact_duration-avg'] = average([c.duration for c in self.contacts]) if len(self.contacts) > 0 else -1

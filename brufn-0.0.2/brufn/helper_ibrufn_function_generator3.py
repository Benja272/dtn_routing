...
class IBRUFNFunctionGenerator:
    ...
    def getRoutingDecisionRng(self, state, node, ts, function) -> Dict[str, List[int]]:
        ...
        for pf in self._failure_probability_rng:
            ...
            if is_next_present:
                # Technically it only has sense if a unique node has all copies
                next_known_state_as_key = ','.join([f'{n+1}:{c}' for n,c in enumerate(next_known_state) if c>0])
                if ts+1 < self._num_of_ts and next_known_state_as_key in function[str(pf)][ts+1].keys():
                    c_ids[str(pf)] += function[str(pf)][ts+1][next_known_state_as_key]
                else:
                    c_ids[str(pf)] += self.getRoutingDecision(next_known_state, node, ts+1, pf=pf)
    ...
    def getRoutingDecision(self, state, node, ts, pf=-1) -> List[int]:
        ...
        for r_decision in routing_decisions if routing_decisions is not None else []:
            ...
            if r_decision['name'] == 'next':
                ...
                next_known_state[self.net.get_contact_by_id(c_id-1).to] += copies_decision
        if is_next_present:
            return c_ids + self.getRoutingDecision(next_known_state, node, ts+1, pf=pf)
